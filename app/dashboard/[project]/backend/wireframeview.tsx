...
export default function WireframeView() {
  const { db_schema, endpoints } = useContext(ProjectMetaContext)!;
    ...
    <div className="flex gap-6 flex-1 min-h-0">
      ...
      <div className="flex flex-col flex-1 min-w-0">
        ...
        <div className="rounded-lg border bg-card p-4 flex flex-col gap-1 flex-1 overflow-auto">
          {endpoints.filter((e) => e.type === "page").length === 0 ? (
            ...
            endpoints
              ...
              .map((ep, i) => (
                <div key={i} className="flex items-center gap-2 text-sm py-1.5 px-2 rounded hover:bg-muted/50 transition-colors">
                  ...
                  <span className="font-mono">{ep.path.replace(/^\//, "")}</span>
                </div>
              ))
          )}
        </div>
      ...
      <div className="flex flex-col flex-1 min-w-0">
        ...
        <div className="rounded-lg border bg-card p-4 flex flex-col gap-1 flex-1 overflow-auto">
          {endpoints.filter((e) => e.type === "api").length === 0 ? (
            ...
            endpoints
              ...
              .map((ep, i) => (
                <div key={i} className="flex items-center gap-2 text-sm py-1.5 px-2 rounded hover:bg-muted/50 transition-colors">
                  <span className={`font-mono text-xs px-1.5 py-0.5 rounded shrink-0 uppercase
                    ${ep.method === "POST" ? "bg-blue-500/10 text-blue-400" :
                      ep.method === "PUT" || ep.method === "PATCH" ? "bg-yellow-500/10 text-yellow-400" :
...
function StatusRow({ label, online }: { label: string; online: boolean }) {
  return (
    <div className="flex items-center justify-between text-sm py-1">
      <span>{label}</span>
      <span className={`text-xs font-medium ${online ? "text-green-500" : "text-muted-foreground"}`}>
