...
// --- Column Row ---
export function ColumnRow({ tableId, col, idx, savedState, tables, onKeyDown, onChange, onToggle, onDelete, onUpdateColumn, onConfirm }: {
  tableId: number;
  col: Column;
  idx: number;
  ...
  return (
    <div className="border-b border-[#e8e4dc] last:border-b-0">
      <div className="flex items-center gap-2 px-4 py-2">
        ...
        {col.pending
          ? <button onClick={onConfirm} className="text-xs px-2 py-1 rounded bg-[#50c878] text-black font-bold">✓</button>
...
// --- Table Card ---
export function TableCard({ table, tables, savedCols, onAddColumn, onAddRow, onDelete, onColumnKeyDown, onColumnChange, onToggleExpanded, onDeleteColumn, onUpdateColumn, onUpdateRowValue, onFetchRows, onConfirmColumn }: {
  table: Table;
  tables: Table[];
  ...
  onUpdateRowValue: (rowIdx: number, colName: string, value: string) => void;
  onFetchRows: () => void;
  onConfirmColumn: (idx: number) => void;
}) {
  return (
