...
export const DBCommandBuilder = {
  ...
  alterTable: (tableName: string, payload: any): string => {
    ...
    switch (action) {
      case "ADD_COLUMN":
        let def = `ALTER TABLE ${tableName} ADD COLUMN ${column.name} ${mapType(column.type)}`;
        if (column.linkedTable) {
          def += ` REFERENCES ${column.linkedTable}(id) ON DELETE CASCADE`;
        }
      return def;
      ...
      case "ADD_FOREIGN_KEY":
        return `ALTER TABLE ${tableName} ADD COLUMN ${payload.column} ${payload.columnType} REFERENCES ${payload.referencedTable}(${payload.referencedColumn})`;
