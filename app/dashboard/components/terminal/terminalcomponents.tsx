...
export function TerminalControls({ start, connect, stop, isRunning, isConnected }: {
  ...
  return (
    <div className="flex gap-2">
      ...
      <button onClick={connect} disabled={isConnected} className="flex items-center gap-1.5 px-4 py-2 rounded font-mono text-sm font-bold disabled:opacity-50 disabled:cursor-not-allowed" style={{ background: "#4a90e0", color: "#fff", border: "2px solid #2a70c0" }}>➤ Connect</button>
      <button onClick={stop} disabled={!isRunning} className="px-4 py-2 rounded font-mono text-lg font-bold disabled:opacity-50 disabled:cursor-not-allowed" style={{ background: "#c85050", color: "#fff", border: "2px solid #47070777" }}>■</button>
    </div>
  )
}
